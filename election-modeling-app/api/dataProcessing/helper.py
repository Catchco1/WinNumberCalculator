...
def intersectDistrictAndPrecinct(path_to_current_districts_shapefile, path_to_old_precinct_shapefile):
    ...
    # create merged dataframe including the intersection shapes between currenDistricts and oldPrecincts. This includes precinct/district combos where the precinct just borders the district
    merged = gpd.overlay(oldPrecincts, currentDistricts, how='intersection')

    # calculate intersection area and divide by total precinct area to find which shapes are actually the full precinct in a district
    merged['area_joined'] = merged.area
    merged['overlapping'] = merged['area_joined'] / merged['precinct_area']

    # the values are not perfect, so anyting with greater than 10% of the area reporting should work. May be worth checking to see that the number of rows in merged after this slicing matches the number of rows in precincts
    merged = merged[merged['overlapping'] > 0.1]
